...
class ArticleSummarizer:
    ...
    def _clean_text(self, text: str) -> str:
        ...
        # Buscar secciones de resultados y conclusiones
        key_sections = []
        ...
        for pattern in section_patterns:
            ...
            for match in matches:
                # Tomar solo las primeras oraciones
                sentences = self._split_into_sentences(match)
                key_sections.extend(sentences[:3])  # Máximo 3 oraciones por sección
    ...
    def _create_accessible_summary(self, text: str, title: str) -> str:
        """
        Crea un resumen accesible sin tecnicismos.
        
        Args:
            text: Texto base para el resumen
        ...
        selected_sentences = self._select_informative_sentences(sentences)
        
        # Simplificar lenguaje técnico
        if self.remove_jargon:
            selected_sentences = [self._simplify_jargon(s) for s in selected_sentences]
        
        # Construir resumen
        summary_parts = []
        
        # Agregar contexto basado en el título
        context = self._generate_context_from_title(title)
        if context:
            summary_parts.append(context)
        
        # Agregar oraciones seleccionadas
    ...
    def _select_informative_sentences(self, sentences: List[str]) -> List[str]:
        """
        Selecciona las oraciones más informativas.
        ...
        important_keywords = [
            ...
            'results', 'findings', 'conclusion', 'significant', 'important',
            'novel', 'new', 'first', 'improved', 'better', 'effective',
            'encontró', 'descubrió', 'mostró', 'demostró', 'reveló',
            'resultados', 'hallazgos', 'conclusión', 'significativo', 'importante',
            'nuevo', 'primera', 'mejorado', 'mejor', 'efectivo'
        ]
        ...
        for sentence in sentences:
            ...
            # Puntuar por longitud (oraciones muy cortas o muy largas son menos útiles)
            word_count = len(sentence.split())
            if 10 <= word_count <= 30:
                score += 1
            
            # Puntuar por posición (primeras oraciones suelen ser más importantes)
        ...
        # Seleccionar hasta 5 oraciones
        selected = [s[0] for s in scored_sentences[:5]]
    ...
    def _generate_context_from_title(self, title: str) -> str:
        """
        Genera contexto introductorio basado en el título.
        ...
        if any(term in title_lower for term in ['bioinformatics', 'bioinformática', 'computational biology']):
            return "Este estudio de bioinformática"
        elif any(term in title_lower for term in ['machine learning', 'deep learning', 'ai', 'artificial intelligence']):
            return "Esta investigación de inteligencia artificial"
        elif any(term in title_lower for term in ['plant', 'planta', 'microbe', 'microbio']):
            return "Este estudio sobre interacciones biológicas"
        elif any(term in title_lower for term in ['education', 'educación', 'teaching', 'enseñanza']):
            return "Esta investigación educativa"
        else:
            return "Este estudio científico"
    
    def _clean_summary(self, summary: str) -> str:
        """
        Limpia y mejora el resumen final.
        
        Args:
            summary: Resumen a limpiar
